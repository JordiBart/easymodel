...
public class SBMLMan {
	...
	public String exportSBML(cat.udl.easymodel.logic.model.Model m, MathLink mathLinkOp) throws Exception {
...
//		p.p("ou "+result);
	...
	public cat.udl.easymodel.logic.model.Model importSBML(ByteArrayInputStream bais, StringBuilder report, String namePrefix)
			throws Exception {
...
//			p.p(r.getReactionStr());
		...
		for (Reaction rs : model.getListOfReactions()) {
			...
			Formula f = new FormulaImpl(
					sessionData.getTempFormulas().getNextFormulaNameByModelShortName(m.getNameShort()), adaptedFormula,
					FormulaType.TEMP, sessionData.getUser(), RepositoryType.TEMP);
			if (!f.isValid())
				throw new Exception("Invalid EM formula " + f.getFormulaDef() + " in reaction " + rs.getId());
			sessionData.getTempFormulas().addFormula(f);
			r.setFormula(f);
//			p.m(f.getGenericParameters());
//			p.p(f.getFormulaDef());
//			p.m2(r.getFormulaValues());
			// set formula parameter values
			formulaParsLoop: for (String par : r.getFormulaValues().keySet()) {
//				p.p(f.getFormulaDef() + " " +rs.getKineticLaw().getFormula()+ " "+par);
				LocalParameter lp = findLocalPar(rs, par);
				if (lp != null) {
					r.getFormulaValues().put(par, new FormulaValueImpl(FormulaValueType.CONSTANT,
...
//								+ " in XMLreaction " + rs.getId());
						...
						for (Compartment c : model.getListOfCompartments()) {
							if (par.equals(getFixedSBMLId(c.getId()))) {
								r.getFormulaValues().put(par, new FormulaValueImpl(FormulaValueType.CONSTANT, "1"));
						...
						for (Species sp : model.getListOfSpecies()) {
							if (par.equals(getFixedSBMLId(sp.getId()))) {
								if (sp.isConstant()
									...
									r.getFormulaValues().put(par, new FormulaValueImpl(FormulaValueType.CONSTANT, "0"));
									continue formulaParsLoop;
								}
//								else
