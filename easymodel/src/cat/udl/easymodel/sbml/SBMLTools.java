...
public class SBMLTools {
	...
	public static String exportSBML(cat.udl.easymodel.logic.model.Model m, MathLinkOp mathLinkOp) throws Exception {
		...
		for (cat.udl.easymodel.logic.model.Reaction r : m) {
			...
			String formula = mathLinkOp.evaluateToString(
					"InputForm[ReleaseHold[Hold[" + r.getFormula().getFormulaWithAddedPrefix("", r.getIdJavaStr())
	...
	public static cat.udl.easymodel.logic.model.Model importSBML(ByteArrayInputStream bais) throws Exception {
		...
		for (Reaction rs : model.getListOfReactions()) {
			...
			Formula f = new FormulaImpl(
					sessionData.getCustomFormulas().getNextFormulaNameByModelShortName(m.getNameShort()),
					FormulaUtils.getSBMLFormulaKeywordFix(rs.getKineticLaw().getFormula()), FormulaType.CUSTOM,
					sessionData.getUser(), RepositoryType.SBML);
			...
			for (String par : r.getFormulaValues().keySet()) {
				...
				if (lp != null) {
					r.getFormulaValues().put(par,
							new FormulaValueImpl(FormulaValueType.CONSTANT, String.valueOf(lp.getValue())));
				} else {
		...
		for (Species sp : model.getListOfSpecies()) {
			...
			m.getAllSpecies().get(sp.getName())
					.setVarType(sp.isConstant() ? SpeciesVarTypeType.INDEP : SpeciesVarTypeType.TIMEDEP);
		}
		return m;
	}
