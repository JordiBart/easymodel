...
public class FormulasEditorVL extends VerticalLayout {
	...
	public FormulasEditorVL(ReactionEditorVL reactionEditorVL) {
		super();
		...
		contentVL.setCaption("Formulas");
		contentVL.setSizeFull();
		contentVL.setDefaultComponentAlignment(Alignment.MIDDLE_CENTER);
		
		Panel contentPanel = new Panel();
		contentPanel.setSizeFull();
		contentPanel.setContent(contentVL);
		contentPanel.setCaption("Velocity Rates");
	...
	private void createGrid() {
		...
		grid.addColumn(Formula::getFormulaDef).setCaption("Rate Definition").setWidth(350);
		...
		}).setCaption("Remove").setWidth(90).setStyleGenerator(item -> "v-align-middle").setWidth(90);
	}
	
	private void updateDisplayContent() {
		contentVL.removeAllComponents();

		createGrid();
		contentVL.addComponent(getHeaderVL());
		contentVL.addComponents(grid, ToolboxVaadin.getTinyWebMathematicaLink());
		contentVL.setExpandRatio(grid, 1.0f);
	}
	...
	private VerticalLayout getHeaderVL() {
		...
		hl11.addComponents(getNewRateButton(),getImportRatesButton());
		InfoWindowButton infoBtn = new InfoWindowButton("How to use Rate editor", "i- How to define rate expressions\r\n" + 
				"    Usable operators: +-/*^()\r\n" + 
				"    Reserved symbols:\r\n" + 
	...
	private Button getImportRatesButton() {
		...
		btn.setIcon(VaadinIcons.HARDDRIVE);		
