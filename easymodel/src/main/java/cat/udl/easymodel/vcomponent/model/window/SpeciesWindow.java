...
//		speciesMap.putAll(model.getAllSpecies());
	...
	private void displayWindowContent() {
		...
		valuesPanelVL.setSpacing(false);
	...
	private HorizontalLayout getSpeciesValueLine(String species, Boolean[] first) {
		...
		speciesValueTF.setPlaceholder("Initial concentration");
		speciesValueTF.setId(species);
		speciesValueTF.setResponsive(true);
		speciesValueTF.addBlurListener(getSpeciesValueTFBlurListener());
		NativeSelect<SpeciesVarTypeType> typeSelect = getTypeNativeSelect(species);
		typeSelect.setSizeFull();
	...
	private NativeSelect<SpeciesVarTypeType> getTypeNativeSelect(String species) {
		...
		ns.setSelectedItem(SpeciesVarTypeType.TIMEDEP);
		ns.setResponsive(true);
		ns.addValueChangeListener(new ValueChangeListener<SpeciesVarTypeType>() {
			private static final long serialVersionUID = 1L;

			@Override
			public void valueChange(ValueChangeEvent<SpeciesVarTypeType> event) {
				NativeSelect<SpeciesVarTypeType> ns = (NativeSelect<SpeciesVarTypeType>) event.getComponent();
