...
public class Models extends ArrayList<Model> {
	...
	public void semiLoadDB() throws DBException {
...
//			pre.setInt(p++, RepositoryType.PUBLIC.getValue());
			...
			while (rs.next()) {
				...
				m.setRepositoryType(RepositoryType.valueOf(rs.getInt("repositorytype")));
				this.addModel(m);
			}
			rs.close();
			pre.close();
	...
	public Model getPrivateModelCopy(Model m, User user) throws Exception {
