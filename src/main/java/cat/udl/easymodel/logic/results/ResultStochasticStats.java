...
public class ResultStochasticStats implements ResultEntry {
    ...
    public ResultStochasticStats(int numTrajectories, boolean isTauLeaping) {
        this.isTauLeaping = isTauLeaping;
        this.numTrajectories = numTrajectories;
        if (!isTauLeaping) {
            int cols = 4, rows = numTrajectories + 2;
            for (int i = 0; i < rows; i++) {
                matrixData.add(new ArrayList<>());
                if (i == 0) {
                    matrixData.get(i).add(new ResultStochasticStatsDataElement("spanBold", "Trajectory"));
            ...
            int cols = 6, rows = numTrajectories + 2;
            for (int i = 0; i < rows; i++) {
                matrixData.add(new ArrayList<>());
                if (i == 0) {
                    matrixData.get(i).add(new ResultStochasticStatsDataElement("spanBold", "Trajectory"));
    ...
    public void updateStochasticProgressBarData(Integer numTrajectory, String newValue) {
        matrixData.get(numTrajectory).get(1).setValue(newValue);
